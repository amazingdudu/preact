...
function createVNode(type, props, key, ref, original) {
	...
	const vnode = {
		...
		// _nextDom必须初始化为未定义的b/c，它最终将被设置为dom。
		// nextSibling，它可以返回' null '，能够区分未初始化的_nextDom和被设置为' null '的_nextDom是很重要的
		_nextDom: undefined,
		_component: null,
		_hydrating: null,
...
function diffChildren(
	...
	for (i = 0; i < renderResult.length; i++) {
		...
		// (holes).
		// 检查是否在oldChildren中找到了对应的元素。
        //如果找到，通过设置为' undefined '删除数组项。
        //我们使用' undefined '，因为' null '是为空占位符(洞)保留的。
...
function setProperty(dom, name, value, oldValue, isSvg) {
	...
	// Benchmark for comparison: https://esbench.com/bench/574c954bdb965b9a00965ac6
		...
		if (typeof value === 'function') ; else if (
			value != null &&
	...
	try {
		outer: if (typeof newType == 'function') {
			...
			if (oldVNode._component) {
				...
				//? 实例化组件时，如果在constructor中把this.props覆盖，再次赋值props,恢复正确的props
				c.props = newProps;
				// 初始化state
				if (!c.state) c.state = {};
			...
			if (newType.getDerivedStateFromProps != null) {
				//? 首次实例化组件时,state与_nextState指向同一个对象
				if (c._nextState == c.state) {
					// 生成新的对象，断开与state之间的联系
					c._nextState = assign({}, c._nextState);
				}

				//调用类组件静态方法getDerivedStateFromProps，合并state
				assign(
					c._nextState,
			...
			if (isNew) {
				//? 当getDerivedStateFromProps不存在时，才调用componentWillMount
				if (
				...
				// 将componentDidMount放入renderCallbacks栈中
				if (c.componentDidMount != null) {
					c._renderCallbacks.push(c.componentDidMount);
				}
			} else {
				//? 当getDerivedStateFromProps不存在时，并且新旧props不同才调用componentWillReceiveProps
				if (
					newType.getDerivedStateFromProps == null &&
					newProps !== oldProps &&
